...
class Game {
    ...
    addConnection = (id) => this.connections.push(id);
    removeConnection = (id) => (this.connections = this.connections.filter((x) => x.id !== id));
    getUsers = () => this.users;
    ...
    isEveryoneReady() {
        ...
        const waiters = this.users.some((user) => !user.state);
    ...
    useVote(voterId, victimId) {
        ...
        if (!voter || !victim || voter.isDead || victim.isDead) {
            return;
        }
    ...
    getMostVotedUser() {
        ...
        const check = this.users.filter(
            (user) => user.id !== mostVoted.id && user.votes >= mostVoted.votes
        );
        if (check.length > 0) return null;
        return mostVoted;
    }

    executeVictim(victim) {
        this.killUser(victim.id);
